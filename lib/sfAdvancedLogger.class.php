    ...
    if (!empty($options['email'])) 
    {
      foreach($options['email'] as $email_log) 
      {
        if (isset($email_log['to'])) {
          if (!is_array($email_log['to'])) {
            $email_log['to'] = array($email_log['to']);
          }
          if (empty($email_log['from'])) {
            $email_log['from'] = 'logger';
          }
          if (empty($email_log['subject'])) {
            $email_log['subject'] = 'Symfony logger';
          }
          if (isset($email_log['type']) && !is_array($email_log['type'])) {
    ...
    if (!empty($options['file'])) 
    {
      foreach($options['file'] as $file_log) 
      {
        if (isset($file_log['file'])) {
          ...
          if (isset($file_log['type']) && !is_array($file_log['type'])) {
